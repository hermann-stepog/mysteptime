...
import { NomeUsuarioField, MotivoField, FormaPagamentoField, useRateioComplementar, RateioComplementarPanel } from "@/components/LogisticaFormFields";
import {
...
function PassagemDialog({ open, onOpenChange, editing, periodosE, colaboradores, unidadeOptions }: {
  ...
  if (open && editing && bound !== editing.id) {
    setF({
      ...
      motivoCancelamento: editing.motivo_cancelamento ?? "", formaPagamento: editing.forma_pagamento ?? "", observacoes: editing.observacoes ?? "",
...
function PassagensAereasPage() {
  ...
  // em vez de já despejar a árvore inteira aberta na tela (pedido dela: "fixa mantém fechada").
                              ...
                              <div key={`${unidadeKey}::sem-bsp`} className="divide-y border-t bg-emerald-50/40 pl-16">
                                {b.itens.map((p) => (
                                  <div
                                    ...
                                    title="Clique para editar esta passagem"
                                  >
